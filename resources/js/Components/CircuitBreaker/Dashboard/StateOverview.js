...
function StateOverview({ tenantId, onServiceSelect }) {
    ...
    useEffect(() => {
        const fetchStates = async () => {
            try {
                setLoading(true);
                const response = await fetch(`/api/web/circuit-breaker/states?tenant_id=${tenantId}`);
                if (!response.ok) {
                        ...
                        React.createElement('div',
                            {
                                ...
                                className: `relative rounded-lg border ${selectedBreaker === breaker.name ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-300'} 
                            ...
                            [
                                ...
                                React.createElement('div',
                                    ...
                                    [
                                        ...
                                        breaker.status === 'OPEN' && breaker.cooldown_until &&
                                        React.createElement('div',
                                            { key: 'cooldown' },
                                            `Retry After: ${new Date(breaker.cooldown_until).toLocaleTimeString()}`
                                        )
                                    ]
                                )
                            ]
