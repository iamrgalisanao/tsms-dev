...
<div class="card">
  <div class="card-header bg-primary">
    <h3 class="card-title text-white">Weekend Sales Report (Sat–Sun)</h3>
  </div>
  <div class="card-body">
    ...
    <div class="table-responsive">
      <table class="table table-bordered table-sm" id="weekly-sales-table" style="font-size: 12px;">
        ...
        <tbody id="weekly-report-tbody">
          <tr>
            <td colspan="13" class="text-center py-4 text-muted">Please select a date range and tenant, then click "Load Report" to view the weekend sales data.</td>
          </tr>
        </tbody>
        <tfoot>
...
$(function() {
  ...
  window.loadWeekend = function(from, to, tenantId) {
    if (!from || !to) { renderEmpty(); return; }
    $.ajax({
      url: '{{ route('commercial.sales-report.tsms-proxy.transactions.weekend') }}',
  ...
  $('#weekly-load-report').on('click', function() {
    ...
    window.loadWeekend(from, to, tenantId);
