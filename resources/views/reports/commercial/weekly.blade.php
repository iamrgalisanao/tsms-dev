...
<div class="card">
  <div class="card-header bg-primary">
    <h3 class="card-title text-white">Weekly Sales / Average Weekly Sales Report</h3>
  </div>
  <div class="card-body">
    ...
    <div class="table-responsive">
      <table class="table table-bordered table-sm" id="weekly-sales-table" style="font-size: 12px;">
        ...
        <tbody id="weekly-report-tbody">
          <tr>
            <td colspan="13" class="text-center py-4 text-muted">Please select a date range and tenant, then click "Load Report" to view the weekly sales data.</td>
          </tr>
        </tbody>
        <tfoot>
...
<script>
$(function() {
  // initialize date inputs with sensible defaults (this week's Monday-Sunday)
  const now = moment();
  ...
  function loadWeekly(from, to, tenantId) {
    if (!from || !to) { renderEmpty(); return; }
    $.ajax({
      url: '{{ route('commercial.sales-report.tsms-proxy.transactions.weekly') }}',
