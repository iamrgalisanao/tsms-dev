...
- **FR-014**: System MUST reconcile inserted rows against the still-present orphans **per day**, using **subset/residual semantics**, before deleting that day's orphans:
  - **Reconciled set**: an inserted row and an orphan match when their (`tax_type`, `amount`) are equal (multiset semantics) **and** `TIMESTAMPDIFF(SECOND, parent_transaction.created_at, orphan.created_at)` falls within **[0, 10] seconds** — measured, not assumed (research.md V5: 99.61% of the post-fix oracle population; one-directional, since a tax row is never created before its parent transaction). Inserted rows' `created_at` MUST be set to the **parent transaction's own `created_at`** — never insertion time (`now()`).
  - **Residual classification** — every orphan not in the reconciled set MUST be classified into exactly one of:
    - `no_replacement_exists`: the orphan's transaction was never reconstructed that day at all (216 transactions, 2026-06-13 only; zero elsewhere) — expected, does not halt.
    - `timestamp_out_of_tolerance`: (`tax_type`, `amount`) matches an inserted row, but the delta exceeds 10 seconds — expected (this is the measured ~0.39% tail), does not halt.
    - `orphan_content_mismatch`: the orphan's transaction WAS reconstructed that day, but no inserted row matches its (`tax_type`, `amount`) at all, regardless of timestamp — this signals the reconstruction itself failed to reproduce the orphan's content, and MUST halt the run before any other day is touched. **Distinct from the pre-existing `cross_check_mismatch` reason code (T013/data-model.md)**, which flags a reconstructed payload disagreeing with the transaction's own stored VAT columns — a different failure mode at a different pipeline stage; the two MUST NOT be conflated in the audit trail.
  - Both `no_replacement_exists` and `timestamp_out_of_tolerance` counts MUST be enumerated exactly (not just "present") before that day's residual is archived (FR-013) and deleted (FR-015/FR-015b). An unclassifiable or miscounted residual is treated as `orphan_content_mismatch` and halts.
