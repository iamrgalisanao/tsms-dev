...
**≈ 89% collapse**, on `$appends` attributes (`net_amount`, `calculated_net_sales`) serialized into **every API response** for the affected transactions — 809,107 of them. *(Corrected 2026-08-10)* `validateAmounts()` assigns `$otherTaxSum` at line 593 but **never uses it** — so the earlier claim of mass validation failure there was wrong. The real validator exposure is `validateAmountReconciliation()` (lines 684-701), whose loop over `$transaction->taxes` is empty today and becomes non-empty after reconstruction — a smaller effect, and it uses the correct exclusion set.
