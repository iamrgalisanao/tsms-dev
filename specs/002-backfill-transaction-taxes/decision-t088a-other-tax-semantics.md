...
The two pre-existing implementations named in the table above — `TSMSTransactionRequest:152` (**live** ingestion gate) and `TransactionValidationService::validateAmountReconciliation()` (**dead**) — both use the same *exclusion* set (`VAT`, `VATABLE_SALES`, `SC_VAT_EXEMPT_SALES`), which still **includes** stray aliases (`VATEXEMPT`, `EXEMPT`, `ZERO_RATED`, `NON-VAT`, `NON_VAT`, `ZERO-RATED`) as "other tax." The PITX formula implies `other_tax` is the `OTHER_TAX` component **only**.

| Choice | Effect |
|---|---|
| **(a) Match existing exclusion set** | Consistent with both pre-existing implementations; lowest risk; but vat-exempt aliases still count as "other tax" in the fixed `otherTaxSum()`, diverging slightly from PITX |
| **(b) Match PITX exactly** — count only `OTHER_TAX`/`OTHER-TAX` | Strictly correct per the business formula; but `otherTaxSum()` then diverges from both pre-existing implementations' broader exclusion set |
...
**This does NOT extend to the live `TSMSTransactionRequest` ingestion gate.** *(Corrected 2026-08-10 — impact review, 5th pass: an earlier draft of this recommendation said "align the other two implementations in the same change," which named the live ingestion gate alongside the dead validator method without distinguishing them — an implementer could have read that as license to change production payload validation as a side effect of this feature.)* Changing `:152`'s exclusion list to an allow-list would alter which payloads pass POS-facing validation for any tax type outside the current list — a live API-contract change with its own blast radius, entirely outside what this feature needs. It is logged as a pre-existing defect for Track B (T088c) instead. Only `validateAmountReconciliation()` (dead) gets the textual reword described in Option 1's Scope, above.

**D2 is scoped accordingly**: "any unknown future tax type MUST NOT silently become `other_tax`" applies to `otherTaxSum()` (the model layer). It does not require or imply changing `TSMSTransactionRequest`'s validation behavior.
