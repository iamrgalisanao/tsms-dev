...
Repair `transaction_taxes` for a 59-day defect window in which every tax row was written with a NULL foreign key. **Insert-first ordering**: archive all 3,238,180 orphan rows, insert correctly-keyed rows reconstructed from retained payloads, reconcile the new rows against the still-present orphans in situ, then delete only the orphan subset whose replacement is proven — leaving 2026-06-13's unrecoverable orphans in place. Finally regenerate the aggregates that genuinely change.
