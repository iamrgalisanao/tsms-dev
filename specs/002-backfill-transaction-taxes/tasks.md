...
- [ ] T068a **BLOCKING, precedes T069.** Run research.md V5's measurement (`TIMESTAMPDIFF(SECOND, t.created_at, tt.created_at)` distribution over post-fix linked rows). Set FR-014's temporal comparison basis from the result — exact-second if the distribution is ~0 everywhere, a stated tolerance otherwise. **Do not implement T069 before this returns**
