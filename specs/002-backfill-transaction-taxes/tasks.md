...
- [ ] T088a-2 Implement the **allow-list** in `app/Models/Transaction.php`: `otherTaxSum()` counts only `OTHER_TAX`/`OTHER-TAX` (D1) and its `sc_vat_exempt_sales` column-fallback is **removed**. Move that deduction into `getNetAmountAttribute()`/`getCalculatedNetSalesAttribute()` as an explicit term — `gross − otherTaxSum() − scVatExemptSales` (D7). Textually align `TransactionValidationService::validateAmounts():593` and `::validateAmountReconciliation():688` with the same allow-list (D4, narrowed per T088a-8 — both methods are unreachable dead code, so this is documentation hygiene, not a behavioral fix), and remove the now-unnecessary subtract-back workaround (the `else` branch at lines 692-698, body 693-697, immediately following the `:688` loop)
