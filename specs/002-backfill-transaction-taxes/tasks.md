...
- [ ] T088a-1 Establish whether any external consumer (POS provider, webapp client) reads the `$appends` attributes `net_amount` / `calculated_net_sales`. **A concrete emitter is now identified** (impact review, 2026-08-10): `POST /api/v1/transactions/{transaction_id}/refund` (`routes/api.php:203` → `TransactionController.php:63`) returns the raw Eloquent model — including both `$appends` — to POS providers, guarded by `auth:sanctum` + `abilities:transaction:create` (`routes/api.php:187,196,203`). Bounds this from an open question to: does anything consuming *that specific endpoint's response* read those two fields
