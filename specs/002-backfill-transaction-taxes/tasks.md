...
- [ ] T070 Implement chunked, **per-day** deletion (FR-015) with predicate strictly `transaction_pk IS NULL`, applied **uniformly to every day in the window, including 2026-06-13** (revised 2026-08-11 — no day-level exception). Every orphan for a day whose reconciliation has passed is deleted — whether classified `no_replacement_exists`, `timestamp_out_of_tolerance`, or matched in the reconciled set (T070a covers 2026-06-13's `no_replacement_exists` specifically) — no per-transaction attribution is attempted for any of them (it is impossible, research.md V4). An unresolved `orphan_content_mismatch` for that day blocks deletion entirely (T069). Bound each chunk by archived id range so it is replayable; assert affected-row counts per chunk; never a single bulk `DELETE`
- [ ] T070a **Archive, verify, then delete ALL of 2026-06-13's `no_replacement_exists` orphans wholesale** (FR-015b, revised 2026-08-11 — reverses the original "retain, never delete" decision). Mark each archived row with reason `no_replacement_exists` and surface in the quarantine report **before** deletion, so the evidence trail exists independent of the live table. Deletion of this residual is gated on two conditions, same as the reconciled set: the residual count verifies exactly 216 transactions' worth of rows for that day (FR-014), and the archive write verifies successful (FR-013). A failure on either blocks deletion — archive-then-delete, never delete-without-verified-archive. No rows are over-retained for the 9 reconstructable transactions on this day; they follow the normal reconciled-set (or `timestamp_out_of_tolerance`) path
- [ ] T071 [P] Feature test: deletion refuses to run when T069 reconciliation has not passed for that day (including an unresolved `orphan_content_mismatch`), never touches a row with non-null `transaction_pk`, and — for 2026-06-13 specifically — refuses to delete the residual if archive-write verification has not independently succeeded (the uniform guardrail replacing the old "never touches 2026-06-13" exception)
