...
- [ ] T088a-2 Implement the **allow-list** in `app/Models/Transaction.php`: `otherTaxSum()` counts only `OTHER_TAX`/`OTHER-TAX` (D1). Converge `validateAmounts():593` and `validateAmountReconciliation():688` on that one shared helper and list (D4), and remove the now-unnecessary subtract-back workaround at lines 693-696
