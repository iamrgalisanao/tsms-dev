...
- [ ] T069 Implement the FR-014 **in-situ reconciliation** (insert-first): after reconstructed rows are inserted — with `created_at` set to **the parent transaction's `created_at`**, never `now()` (T068a) — match them against the still-present orphans per **(`tax_type`, `amount`) multiset**, evaluated **per day**, where a match additionally requires `TIMESTAMPDIFF(SECOND, parent.created_at, orphan.created_at)` in **[0, 10]** (T068a/FR-014). Every unmatched orphan MUST be classified into exactly one of `no_replacement_exists` / `timestamp_out_of_tolerance` / `orphan_content_mismatch` (FR-014) and the classification recorded (this is what the archive table's `reason_code` column, added in Slice 12/T066, is for — Stage 1 left it `NULL` for every row precisely so this task can populate it). `no_replacement_exists` and `timestamp_out_of_tolerance` are expected and do not halt; `orphan_content_mismatch` halts before any other day is touched. Proves *content*, not attribution — attribution is carried by T023's post-fix oracle alone, and both must pass
