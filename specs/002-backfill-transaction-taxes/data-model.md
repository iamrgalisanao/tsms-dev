...
| archive/reconciliation references | **`archive_reference` remains an unpopulated placeholder column as of Slice 14 (2026-08-12) — no code path writes to it.** Stage 2 (`OrphanTaxReconciler`) and Stage 3 (`OrphanTaxDeleter`) never join back to `tax_backfill_records` at all; they operate entirely on `transaction_taxes_orphan_archive`'s own `reconciled_status`/`reason_code` columns, matched by `original_id`, day-scoped rather than per-transaction — consistent with this feature's "no per-row attribution" design (research.md V4). This field's originally-anticipated shape (a day/run-level reference, per the Slice 2 drift-revalidation note this row previously described) turned out not to be needed by either implementation; it is retained on the schema but genuinely inert. |
...
| Concern | Requirement |
|---------|-------------|
| Archive | Durable, queryable copy of **all** orphans preserving `id`, `tax_type`, `amount`, timestamps (FR-013), plus a `reconciled_status` (`reconciled` / `residual`, populated once reconciliation passes for that row's day) and — for every `residual` row, not only FR-015b's — a `reason_code` (`no_replacement_exists`, `timestamp_out_of_tolerance`, or `orphan_content_mismatch`) (FR-013 extended 2026-08-11 — see `tasks.md` T066) |
| Ordering | **Insert-first** — archive, insert, reconcile in situ, then delete (FR-015a) |
| Reconciliation | **Revised 2026-08-12 (T068a's measurement, research.md V5) — not an exact-second multiset match.** An inserted row and an orphan match when their (`tax_type`, `amount`) are equal and `TIMESTAMPDIFF(SECOND, parent.created_at, orphan.created_at)` falls in **[0, 10] seconds** (one-directional), evaluated per day via per-bucket matching — never per-row transaction attribution (research.md V4). Every unmatched orphan is classified `no_replacement_exists`, `timestamp_out_of_tolerance`, or `orphan_content_mismatch` (FR-014); only the last halts. Inserted `created_at` = **the parent transaction's `created_at`**, never `now()`. Proves **content**, never **attribution** — attribution rests solely on the post-fix oracle (R4) |
| Deletion scope | **Revised 2026-08-11 — all orphans, every day, uniformly.** 2026-06-13's 216 unrecoverable transactions' orphans are archived (their only surviving evidence, per research.md's V1a/V1b consequences) and then **deleted from the live table**, same as every reconciled or `timestamp_out_of_tolerance` day — no code-level distinction between the three non-halting classifications. Deletion is gated on three enforced conditions (T079): archive completeness for the day, every archived row for the day carrying a non-NULL `reconciled_status` (i.e. reconciliation passed), and a supplied `--token` matching a day-bound sha256 hash recomputed server-side from that persisted state — no day-level exception remains |
...
| Rollback | A bad insert rolls back without touching the archive, since originals are still present. Restoring a bad delete requires re-inserting from the archive (both the reconciled-day rows and, if ever needed, the 216's archived rows) — this feature builds no automated restore command; recovery is a manual re-insert from `transaction_taxes_orphan_archive` |
