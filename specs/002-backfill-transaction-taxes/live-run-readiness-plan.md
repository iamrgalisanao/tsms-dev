...
**Update 2026-08-12 (Slice 16)**: `CaptureIntegrityEvidence` (T075/T099) is now built, tested, code-reviewed, and architect-revalidated — §5 below is no longer blocked for the pre-run half. It found and fixed a real correctness trap: the T062 duplicate-check query, applied naively, would have let MySQL's NULL-grouping behavior collapse the 3.24M-row orphan population into meaningless phantom "duplicate" groups — `WHERE transaction_pk IS NOT NULL` is now mandatory and regression-tested. The `post_run` phase is invoked identically (`--phase=post_run --apply`), as a required, separate operator step after the backfill/reconcile/delete/refresh sequence, before T076.
...
**Update 2026-08-13 (Slice 19, T047/Command 3 — materiality report)**: `transactions:tax-backfill-materiality` (T046-T051) is now built, tested, code-reviewed, and architect-revalidated — SC-006's "reproducible before/after list" dependency is closed. Compares Slice 15's pinned `before` snapshot against a freshly captured, equally pinned `after` render (same `SalesReportDataService::getCmsrReportData()` path), per (tenant, reporting month), on the `other_tax` component specifically. Threshold evaluation (FR-009a) is read-time only — the pass/fail flag is never persisted, so finance can retune `--threshold-amount`/`--threshold-percent` later against the same immutably-captured deltas without any new report-path call. Per-pair source-mismatch isolation (FR-012a): one tenant's source flip is quarantined (`comparison_status = source_mismatch`) without blocking visibility into every other tenant. Two non-blocking notes from the architect drift-revalidation: (a) a narrow, benign race exists when `--snapshot-run` is omitted — two concurrent default-resolution invocations could pick different (both valid, both correctly labeled) snapshot-run baselines if a new Slice 15 snapshot completes in between; pass `--snapshot-run` explicitly to eliminate it, or accept it as documented; (b) same `CACHE_DRIVER` requirement as Slice 15 (redis/database/memcached for real cross-process locking, not `file`/`array`). The materiality-list-informed finance handoff note (T063) remains separately scoped, unbuilt work.

**Update 2026-08-13 (Slice 20, T076 — final evidence-gate / readiness verdict)**: `transactions:tax-backfill-readiness-verdict` is now built, tested, code-reviewed, and architect-revalidated — this closes the feature's readiness-plan checklist entirely (§12 below). Redefined by explicit architect/product-owner direction as the feature's final evidence-gate slice rather than the narrower original T076 wording: a **pure reader**, zero writes under any code path, that pulls together Slice 16's pre/post-run integrity captures, Slice 19's materiality evidence, Slice 18's connection-identity evidence, and a manual `--backup-drill-confirmed` attestation for T054 (no DB signal exists for that fact) into one `pass`/`warn`/`fail` verdict for the rehearsal/live-run record. Materiality is informational only (WARN, never FAIL) — a flagged tenant is the expected outcome of a real correction. The duplicate-check comparison remains a secondary signal (Architect F8): FAIL only if post-run duplicates exceed the pre-run baseline. **T076 never invokes any capture mechanism itself** — `transactions:capture-integrity-evidence --phase=post_run --apply` is a required, separate, prior operator step (§4 below, updated). A small structured-field enrichment (`transaction_taxes_null_count`) was added to Slice 16's capture command in this same slice so the verdict never has to parse `txn:pk-integrity`'s verbatim text output.
...
Coverage (Step 6), tenant/tax-type/totals validation (T058-T061, still unbuilt), duplicate check (T062 — compared against T075's pre-run baseline, captured via Slice 16's `transactions:capture-integrity-evidence`). `transaction_pk IS NULL` count in `transaction_taxes` must be **zero** (revised: FR-015b means zero is the correct end state for the whole window, not just reconciled days) — this and the duplicate-check comparison are now assembled automatically into a `pass`/`warn`/`fail` verdict by `transactions:tax-backfill-readiness-verdict` (T076, built Slice 20, §4 step 11), alongside materiality and connection-identity evidence.
...
1. `php artisan transactions:capture-integrity-evidence --from=... --to=... --phase=pre_run --apply` (baseline — built, Slice 16; embeds `txn:pk-integrity` verbatim plus a structured `transaction_taxes_null_count`, added Slice 20)
...
10. `php artisan transactions:capture-integrity-evidence --from=... --to=... --phase=post_run --apply` (post-run — built, Slice 16/20; a required, separate step, never invoked by step 11)
11. `php artisan transactions:tax-backfill-readiness-verdict --from=... --to=... --snapshot-run=<ID> --pre-run-capture-id=<ID> --post-run-capture-id=<ID> --backup-drill-confirmed --json` (built, Slice 20) — the final `pass`/`warn`/`fail` verdict for the rehearsal/live-run record; pure reader, never writes anything

## 5. Pre-run evidence to capture

- `txn:pk-integrity` output, before and after (T099) — captured verbatim (plain text) via `php artisan transactions:capture-integrity-evidence --from=... --to=... --phase=pre_run|post_run --apply` (Slice 16, built 2026-08-12; both phases required — run pre_run before mutation and post_run after, per §4 steps 1 and 10).
...
- Duplicate-check baseline and `transaction_taxes_null_count` (T075, extended Slice 20) — built: the same command above persists the corrected (`WHERE transaction_pk IS NOT NULL`) `GROUP BY (transaction_pk, tax_type)` result plus a structured NULL-count, so T076's readiness verdict has a real, structured pre/post-run comparison instead of an assumed zero or a parsed text report.
...
- [x] T047/Command 3 — materiality report command exists and is tested (Slice 19, 2026-08-13); closes SC-006
- [x] T076 — final evidence-gate / readiness verdict command exists and is tested (Slice 20, 2026-08-13); pure reader, assembles Slice 16/18/19 evidence into a `pass`/`warn`/`fail` verdict
