# TSMS POS Integration Guidelines (V2.2 Final)

> [!TIP]
> **Developer Quick Start (2-Minute Summary)**:
> 1. **Auth**: Get your Bearer Token and use it for all requests.
> 2. **Structure**: Send exactly **one** transaction per request (Batch is Phase 2).
> 3. **Hashing**: Implement **recursive alphabetical sorting (ksort)** and **strict 2-decimal string formatting**.
> 4. **Validation**: Check your `transaction.payload_checksum` (Inner) and root `payload_checksum` (Outer).
> 5. **Confirmation**: Use the status endpoint to verify background ingestion result.
...
| `submission_timestamp` | Yes | ISO-8601 | Time the API call was initiated. |
...
- **Allowed Examples**: `2026-03-16T13:14:17Z` or `2026-03-16T13:14:17.681Z`.
- UTC format with the `Z` suffix is strongly preferred.
...
| :--- | :--- | :--- |
| 2026-03-28 | 2.1 | **Production Synchronization**: Corrected stale V2.1 sample hashes to match SHA-256 production logic. |
| 2026-03-28 | 2.1 | **Refund Logic Enhancement**: Implemented amount validation (≤ net sales) and set `refund_reference` as system-assigned `transaction_id`. |
| 2026-03-28 | 2.1 | **Status Parity**: Aligned Job Status documentation with production constants (`QUEUED`, `FAILED`). |
| 2026-03-28 | 2.2 | **Final Specification Promotion**: Unified all V2.1 feedback into official V2.2 standard. Validated all sample Golden Records. |
